...
| **exp055** transition-as-rule-change (qualified positive) | Lift the compounding law to the *tower*: run exp053 at each level and DERIVE each level's law from the level below's competence (a major transition as a rule-change). Does competence compound *across* levels? | **Yes, modestly — a first across-level meta-ratchet, but fragile.** On full-depth towers the **derived-law** arm rises tier-over-tier **1.78 → 1.89 → 1.94** (+**0.081**/tier) where the *fixed* compounding law is flat (+0.006) and self-similar is +0.010 — so the competence-derived **rule-change** (not the richer law alone) is what tilts the across-level slope, and every tier sits far above the self-similar tower (~1.8 vs ~1.25). **But it costs robustness:** ~**⅓ of compounding towers collapse** (competence selection thins the collective *diversity* the transition needs) vs **0%** for self-similar — the exp047 tension at the tower scale. Competence compounds within a level (exp053) and modestly across levels (exp055), bounded by a competence-vs-diversity trade-off. *(The naive per-tier average is survivorship-biased; metrics are full-depth-only + collapse rate.)* |
