...
| **Ω-0.50** the world census | Does the living world sustain OEE + life at scale, and how deep does the tower climb? | **It stays alive, open, and competent — with an honest bound on competence.** A **250k-tick × 4-seed census** (`studies/exp061_census.html`): competence holds a stable **~1.9 plateau**, the *genuine* eviction-robust novelty rate holds a **positive floor ~0.24/tick** (62.5k distinct classes ever), and self-maintaining lifeforms **persist and grow** (21 → 24) — the world does *not* decay. A live tower pushing 16 tiers reached **depth 16/16** on one seed (**no intrinsic depth ceiling** in range; mean 6.5), but per-tier competence is a **flat ~1.85 plateau** — depth is unbounded, competence **saturates**. So the world is open-ended in *construction and individuality* indefinitely and competent to a high stable level, but **not unboundedly self-improving** — a precise statement of what the substrate does. |
| **Ω-0.56** why competence saturates across levels | Does the census plateau come from *opacity at the promotion boundary* — and does making it transparent let competence compound up the tower? | **No — the ceiling is intrinsic, not a boundary artifact (an honest negative, with a bonus).** Making promotion **transparent** (carry the finishing tier's network-visible catalytic repertoire across the boundary + seed the child with its product states — the exp053 fix one level up) does **not** raise the across-tier competence slope (transparent **+0.008** vs the scalar-law tower's +0.024; competence stays on the ~1.9 plateau at every tier). So the cap is the **per-tier substrate's optimal-richness ceiling** (exp053/054), not what promotion carries — the within-level "rate up to optimal richness, then a stock" law holds at tower scale and is **boundary-independent**. Bonus: transparent promotion **halves tower collapse** (0.40 → 0.20) and **deepens** it (3.2 → 4.0 tiers) — carrying competent structure aids *bootstrapping*, not the ceiling. Next lever: a rising target that forces each tier to *exceed* the last. `carry_catalysts=False` ⇒ byte-identical. |
| **Ω-0.57** is the plateau satisficing or a hard ceiling? | If transparency can't lift it, can a *rising target* — carry the competence bar across the boundary so each tier must *exceed* the last? | **No — it's a HARD expressivity ceiling; a rising target makes competence *fall* (a decisive negative that, with Ω-0.56, triangulates the limit).** Seeding tier N+1's moving competence bar from tier N's achievement turns the across-tier slope **negative** (−0.036 vs the reset +0.010) and **raises collapse** (0.60 vs 0.40): when a tier can't build past ~1.9, *no deme clears the seeded bar*, selection drops to the floor → drift → competence erodes and towers die. So cross-level competence is capped by **what each tier can express**, not by the promotion boundary (Ω-0.56) *or* the selection target (this) — two of three levers ruled out. The one remaining: a **richer per-tier construction law** (change what a tier can *build*). `carry_ratchet_bar=False` ⇒ byte-identical. |
| **Ω-0.58** the last lever — a richer per-tier law | Change *what each tier can express* — derive each tier's construction reach from the competence below. Does competence finally climb up the tower? | **No — it starves cross-production and *collapses* the tower; the ceiling is a HARD bound and the three-lever triangulation is complete.** A deeper earned construction law makes **every** tower collapse before full depth (collapse **1.00** vs 0.40, depth 2.4 vs 3.2) — the exp054 depth-trap (deeper reach starves the closure competence rewards) now compounding *up* the tower; where towers stay shallow enough to survive, competence plateaus flat ~1.9 (no climb). So **all three cross-tier levers fail** — carry structure (Ω-0.56), raise the target (Ω-0.57), enrich the law (this): **cross-level competence compounding is impossible in this substrate.** The world is open-ended in *construction and individuality* indefinitely and competent to a high stable level, but **competence is a STOCK, not a rate, at world scale** — a hard, falsifiable close of the self-improvement arc (exceeding it needs a *new substrate*, not a new lever). `resolution_from_competence=False` ⇒ byte-identical. |
| **Ω-0.51** minds — is *agency* selectable? | Are the world's collectives *agents* — do they perceive their environment, act on it, and get selected for it? | **Yes — the first rung of the embodiment thread.** An **agent** is a collective with a heritable **perception→action policy**: it senses the environment's season (the cyclic feed) and forages the band its policy targets. Against a **perception-ablated "blind" control** (same action, a percept decoupled from the real world), embodied agents' policies **concentrate at the informative phase** (φ=1: 0.47→0.76 with action strength, vs blind drifting to 0.06) — perception is *used and selected* — and their anticipation advantage over blind **grows dose-dependently** with action strength (+0.02 → **+0.17**). Sensing-and-acting is a selectable, behaviourally-advantageous trait: the minimal proto-mind. The world is not just alive — its collectives are agents. `agent_policy=""` ⇒ byte-identical. |
| **Ω-0.52** minds — does *locomotive* agency pay? | Is perception-directed *movement* (taxis) selectable too, like perception-directed construction (exp062)? | **No — an honest negative that bounds embodiment.** A **taxis** agent that perceives which neighbour patch is richest in the next season's band and migrates toward it is **worse** than one that moves at random (anticipation 0.29–0.31 vs blind **0.43**), across two designs (greedy *and* crowding-aware ideal-free). The mechanism: random movement already gives the optimal even spatial spread (24/24 patches), so directed movement only **herds** agents onto shared "best" patches (18/24), and the resource is **self-generated and local** (each agent forages it into its *own* patch), so relocating *abandons* self-built structure. **Agency that BUILDS (exp062) pays; agency that RELOCATES does not, in a niche-constructing world.** Predicted follow-on: an *exogenous* patchy resource should make taxis pay. |
| **Ω-0.53** minds — does an *exogenous* resource rescue taxis? | exp063 predicted taxis would pay if the resource genuinely lived *elsewhere*. Does it? | **No — the predicted positive is refuted, deepening the negative.** With the *environment* injecting the resource into rotating patches (a dose-response), no taxis policy beats random dispersal, and taxis gets **worse** as the resource strengthens (taxis−blind gap **+0.00 → −0.20**): a stronger resource lifts blind steeply (0.28→0.57) but not taxis. **Random dispersal already realizes the ideal-free distribution** (even spread, 24/24 patches, harvests a rotating resource optimally); directed movement **herds** agents onto the resource (18/24), and even per-capita "ideal-free" taxis herds because all agents best-respond to the *same stale state at once*. Locomotive agency fails not because of *where* the resource is but because **shared-perception simultaneous movement is anti-cooperative**. Next probe: private/asynchronous perception, which should break the herd. |
| **Ω-0.54** minds — does *memory* pay? (reflex → cognition) | Is an agent that acts on the *history* of its percepts (internal state) selectable when the instant percept is insufficient? | **Yes — and only when the world requires it (a double dissociation).** In a **partially-observable** triangle season (0,1,2,3,2,1,0,… — the current band no longer implies the next), a **memory** agent that carries the last season *direction* and extrapolates it **out-anticipates** the reactive agent (0.364 vs 0.320); in the **instant-observable** sawtooth it does *not* (0.290 vs 0.391 — the wrap confuses the extrapolator). Memory − reactive = **+0.044 (triangle) vs −0.101 (sawtooth)**, an **interaction of +0.145**. Because the benefit is an *interaction*, not a main effect, it is **memory itself** that is selected — the embodiment thread's step from *reflex* to *cognition*: internal state that integrates the past is selectable exactly when the instant percept underdetermines the right action. `agent_policy=""` / `season_pattern="sawtooth"` ⇒ byte-identical. |
| **Ω-0.55** minds — does *multi-cue* perception pay? (breadth) | Can an agent integrate *two* observable cues, and is it cue **integration** that pays or just a richer policy? | **Yes — integration pays iff the cue is relevant, and an irrelevant cue even COSTS (a genome-matched double dissociation).** When the reward is a **conjunction** of the season and a second observable "regime" cue, a **multi**-cue agent (per-regime action table) out-anticipates a **matched cue-blind** control — same table genome, but percept of the cue decoupled — by **+0.097**; when the cue varies but is irrelevant it is **worse** by **−0.095** (conditioning on noise fragments the policy), an **interaction of +0.193**. Crucially, the naive contrast vs a *scalar* agent shows a **spurious main effect** (the richer table genome helps in both worlds, +0.080); only the matched control isolates that it is **integration**, not extra parameters. A second route to cognition — **breadth** — complementing Ω-0.54's memory (**depth in time**). `agent_policy=""` / `env_cues=1` ⇒ byte-identical. |
...
python -m unittest discover -s omega/tests  # run the test suite (97 tests)
```
...
history in [`omega/docs/RESEARCH_LOG.md`](omega/docs/RESEARCH_LOG.md) (milestones Ω-0.1 → Ω-0.58).
